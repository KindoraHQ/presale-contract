...
contract KNRStagePresale is Ownable, ReentrancyGuard {
    ...
    function setMarketingWallet(address _marketingWallet) external onlyOwner onlyBeforeStart {
        require(_marketingWallet != address(0), "MKT_ZERO"); // Fixed line
        marketingWallet = _marketingWallet;
    }
    ...
    function finalize() external nonReentrant {
        ...
        if (hardCapWei != 0) {
            uint256 maxLpWei = (hardCapWei * lpPercent) / 100;
            uint256 requiredLpTokens = (maxLpWei * listingRate) / 1e18;
            require(
                saleToken.balanceOf(address(this)) >= saleAllocation + requiredLpTokens,
                "NEED_TOKENS_FOR_LP"
            );
        }
